...
void report_time(int dt,int nt,float base,unsigned int nrand)
{
  printf("ti=%f pen=%f rand=%d\n",((float) dt)/CLOCKS_PER_SEC/nt*1000,dt/base,nrand/nt);
}
...
int runalgo(void (*algo)(byte *,byte *,byte *),byte *in,byte *out,byte *key,byte *outex,int nbyte,int nt,float base)
{
  int i;
  clock_t start,end;
  int dt;
