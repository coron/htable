// This program is free software; you can redistribute it and/or modify it
// under the terms of the GNU General Public License version 2 as published
// by the Free Software Foundation.

#include "aes_rp.h"
#include "aes.h"
#include "share.h"

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <assert.h>

byte sq[256]={
};

byte taffine[256]={
};

byte tsmult[1024]={
};

void gensquare()
{
  int i;
  byte x=0;
  printf("byte sq[256]={");
  for(i=0;i<256;i++)
  {
    if((i%8)==0) printf("\n");
    printf("0x%02x",mult(x,x));
    x++;
    if(i<255) printf(",");
  }
  printf("};\n");
}

byte square(byte x)
{
  //return mult(x,x);
  return sq[x];
}

void gentaffine()
{
  int i;
  byte x=0;
  printf("byte taffine[256]={");
  for(i=0;i<256;i++)
  {
    if((i%8)==0) printf("\n");
    printf("0x%02x",affine(x));
    x++;
    if(i<255) printf(",");
  }
  printf("};\n");
}

void gensmall_multtable()
{
  int i,j;
  byte x,y;
  byte t[1024];
  for(i=0;i<2;i++)
    for(j=0;j<2;j++)
      for(x=0;x<16;x++)
	for(y=0;y<16;y++)
	  t[(i << 9) | (j << 8) | (x << 4) | y]=mult(x << (i*4),y << (j*4));
   printf("byte tsmult[1024]={");
   for(i=0;i<1024;i++)
   {
      if((i%8)==0) printf("\n");
      printf("0x%02x",t[i]);
      if(i<1023) printf(",");
   }
   printf("};\n");
}

// Computes z=x*y in GF(2^8) using four 8-bit tables 
byte multtable(byte x,byte y)
{
  return tsmult[0   | ((x & 15) << 4) | (y & 15)] ^
         tsmult[512 | (x & 240) | (y & 15)] ^
         tsmult[256 | ((x & 15) << 4) | (y >> 4)] ^
         tsmult[768 | (x & 240) | (y >> 4)];
}

// AES Sbox computation without masking
byte subbyte_rp(byte x)
{
  byte u2=square(x);
  byte u3=multtable(u2,x);
  byte u6=square(u3);
  byte u7=multtable(u6,x);
  byte u14=square(u7);
  byte u15=multtable(u14,x);
  byte u240=square(square(square(square(u15))));
  byte u254=multtable(u240,u14);
  return taffine[u254];
}

void square_share(byte *a,int n)
{
  int i;
  for(i=0;i<n;i++)
    a[i]=square(a[i]);
}

void square_share_s(byte *a,int *ia,byte *z,int *iz,int k,int ell)
{
  memcpy(z,a,k*ell);
  memcpy(iz,ia,k*sizeof(int));
  square_share(z,k*ell);  
}

// The shared multiplication SecMult of Rivain-Prouff
void multshare(byte *a,byte *b,byte *c,int n)
{
  // Memory: 4 bytes
  int i,j; 
  for(i=0;i<n;i++)
    c[i]=multtable(a[i],b[i]);

  for(i=0;i<n;i++)
  {
    for(j=i+1;j<n;j++)
    {
      byte tmp=xorshf96(); //rand();
      byte tmp2=(tmp ^ multtable(a[i],b[j])) ^ multtable(a[j],b[i]);
      c[i]^=tmp;
      c[j]^=tmp2;
    }
  }
}

void mult_gate_shuffling(byte *a,int ia,byte *b,int ib,byte *c,int *ic,int ell)
{
  *ic=xorshf96() % ell;
  byte ta[ell];
  rotate_shuffling(a,ta,*ic-ia,ell);
  byte tb[ell];
  rotate_shuffling(b,tb,*ic-ib,ell);
  for(int i=0;i<ell;i++)
    c[i]=multtable(ta[i],tb[i]);
}

void test_mult_gate_shuffling(int ell)
{
  byte a[ell],b[ell],c[ell];
  byte va=45,vb=34;
  int ia=0,ib=0,ic;

  share(va,a,ell);
  reshuffle(a,&ia,ell);
  
  share(vb,b,ell);
  reshuffle(b,&ib,ell);
  
  mult_gate_shuffling(a,ia,b,ib,c,&ic,ell);

  assert(c[ic]==multtable(va,vb));
}


void random_gate_shuffling(byte *a,int *ia,int ell)
{
  for(int i=0;i<ell;i++)
    a[i]=xorshf96();
  *ia=xorshf96() % ell;
}

void random_gate_shuffling_noind(byte *a,int ell)
{
  for(int i=0;i<ell;i++)
    a[i]=xorshf96();
}

// The shared multiplication SecMult of Rivain-Prouff
void multshare_s_slow(byte *a,int *ia,byte *b,int *ib,byte *c,int *ic,int k,int ell)
{
  for(int i=0;i<k;i++)
    mult_gate_shuffling(a+i*ell,ia[i],b+i*ell,ib[i],c+i*ell,ic+i,ell);

  for(int i=0;i<k;i++)
  {
    for(int j=i+1;j<k;j++)
    {
      byte tmp[ell];
      int itmp;
       random_gate_shuffling(tmp,&itmp,ell);

      byte tmp2[ell],aibj[ell],ajbi[ell];
      int itmp2,iaibj,iajbi;
      
      mult_gate_shuffling(a+i*ell,ia[i],b+j*ell,ib[j],aibj,&iaibj,ell);
      xor_gate_shuffling(tmp,itmp,aibj,iaibj,tmp2,&itmp2,ell);

      mult_gate_shuffling(a+j*ell,ia[j],b+i*ell,ib[i],ajbi,&iajbi,ell);
      xor_gate_shuffling(tmp2,itmp2,ajbi,iajbi,tmp2,&itmp2,ell);

      xor_gate_shuffling(c+i*ell,ic[i],tmp,itmp,c+i*ell,ic+i,ell);
      xor_gate_shuffling(c+j*ell,ic[j],tmp2,itmp2,c+j*ell,ic+j,ell); 
    }
  }
}

// The shared multiplication SecMult of Rivain-Prouff
void multshare_s(byte *a,int *ia,byte *b,int *ib,byte *c,int *ic,int k,int ell)
{
  for(int i=0;i<k;i++)
    mult_gate_shuffling(a+i*ell,ia[i],b+i*ell,ib[i],c+i*ell,ic+i,ell);

  for(int i=0;i<k;i++)
  {
    for(int j=i+1;j<k;j++)
    {
      byte tmp[ell];
      int itmp=ic[i];
      random_gate_shuffling_noind(tmp,ell);

      byte tmp2[ell],aibj[ell],ajbi[ell];
      int itmp2,iaibj,iajbi;
      
      mult_gate_shuffling(a+i*ell,ia[i],b+j*ell,ib[j],aibj,&iaibj,ell);
      xor_gate_shuffling(tmp,itmp,aibj,iaibj,tmp2,&itmp2,ell);

      mult_gate_shuffling(a+j*ell,ia[j],b+i*ell,ib[i],ajbi,&iajbi,ell);
      xor_gate_shuffling(tmp2,itmp2,ajbi,iajbi,tmp2,&itmp2,ell);

      xor_gate_shuffling_noind(c+i*ell,tmp,c+i*ell,ell);

      byte tmp3[ell];
      int itmp3=ic[j];
      rotate_shuffling(tmp2,tmp3,itmp3-itmp2,ell);
      xor_gate_shuffling_noind(c+j*ell,tmp3,c+j*ell,ell); 
    }
  }
}

void test_multshare_s(int k,int ell)
{
  byte a[ell*k],b[ell*k],c[ell*k];
  int inda[k],indb[k],indc[k];

  byte va=10,vb=45;
  encode_s(va,a,inda,k,ell);
  encode_s(vb,b,indb,k,ell);
  
  multshare_s(a,inda,b,indb,c,indc,k,ell);

  assert(decode_s(c,indc,k,ell) == multtable(va,vb));
}

void subbyte_rp_share_func(byte *a,int n,void (*multshare_call)(byte *,byte *,byte *,int))
{
  // Memory: 5*n+5 byte 
  int i;
  byte z[n],z2[n];
  byte one[n];

  for(i=1;i<n;i++)
    one[i]=0;

  one[0]=1;

  memcpy(z,a,n);
  square_share(z,n);    // z=x^2     

  multshare_call(z,one,z2,n);  // z=Refresh(z)
  memcpy(z,z2,n);

  byte y[n];
  multshare_call(z,a,y,n);   // y=z*x=x^3

  byte w[n];
  memcpy(w,y,n);
  square_share(w,n);
  square_share(w,n);     // w=x^12

  byte w2[n];
  multshare_call(w,one,w2,n); // w=Refresh(w)
  memcpy(w,w2,n);

  byte y2[n];
  multshare_call(y,w,y2,n);   // y2=x^15

  square_share(y2,n);
  square_share(y2,n);
  square_share(y2,n);
  square_share(y2,n);    // y2=x^240
  
  multshare_call(w,y2,y,n);   // y=x^252
  multshare_call(y,z,a,n);    // a=x^254

  for(i=0;i<n;i++)
    a[i]=taffine[a[i]];
  if((n%2)==0)
    a[0]^=99;
}

void subbyte_rp_share(byte *a,int n)
{
  subbyte_rp_share_func(a,n,multshare);
}

void refresh_sni_s(byte *a,int *ia,int k,int ell)
{
  byte one[k*ell];
  int ione[k];

  share(1,one,k*ell);
  for(int i=0;i<k;i++) 
    ione[i]=0;

  byte z[k*ell];
  int iz[k];

  multshare_s(a,ia,one,ione,z,iz,k,ell);

  memcpy(a,z,k*ell);
  memcpy(ia,iz,k*sizeof(int));
}

void test_refresh_sni_s(int k,int ell)
{
  byte a[ell*k];
  int ia[k];

  byte va=10;
  encode_s(va,a,ia,k,ell);

  refresh_sni_s(a,ia,k,ell);

  assert(decode_s(a,ia,k,ell) == va);
}

void subbyte_rp_share_s(byte *a,int *ia,int k,int ell)
{
  byte z[k*ell];
  int iz[k];

  square_share_s(a,ia,z,iz,k,ell); // z=x^2
  refresh_sni_s(z,iz,k,ell);       // z=refresh(z)

  byte y[k*ell];
  int iy[k];
  multshare_s(z,iz,a,ia,y,iy,k,ell);  // y=x^3

  byte w[k*ell];
  int iw[k];
  square_share_s(y,iy,w,iw,k,ell);
  square_share(w,k*ell);                  // w=x^12
  refresh_sni_s(w,iw,k,ell);
  
  byte y2[k*ell];
  int iy2[k];
  multshare_s(y,iy,w,iw,y2,iy2,k,ell);  // y2=x^15

  square_share(y2,k*ell);
  square_share(y2,k*ell);
  square_share(y2,k*ell);
  square_share(y2,k*ell);                // y2=x^240

  multshare_s(w,iw,y2,iy2,y,iy,k,ell);
  multshare_s(y,iy,z,iz,a,ia,k,ell);

  for(int i=0;i<k*ell;i++)
    a[i]=taffine[a[i]];

  if((k%2)==0)
  for(int i=0;i<ell;i++)
    a[i]^=99;
}  

void test_subbyte_rp_share_s(int k,int ell)
{
  byte a[ell*k];
  int ia[k];

  byte va=10;
  encode_s(va,a,ia,k,ell);

  subbyte_rp_share_s(a,ia,k,ell);

  assert(decode_s(a,ia,k,ell) == subbyte_rp(va));
}


// AES with RP Sbox computation without masking
void subbytestate_rp(byte *state)
{
  int i;
  for(i=0;i<16;i++) state[i]=subbyte_rp(state[i]);
}

// AES with RP Sbox computation without masking
void aes_rp(byte in[16],byte out[16],byte key[16])
{
  int i,j;
  int round=0;
  byte state[16];
  byte w[176];

  keyexpansion(key,w);

  for(i=0;i<16;i++)
    state[i]=in[i];

  addroundkey(state,w,0);

  for(round=1;round<10;round++)
  { 
    subbytestate_rp(state);
    shiftrows(state);
    mixcolumns(state);
    addroundkey(state,w,round);
  }
 
  subbytestate(state);
  shiftrows(state);
  addroundkey(state,w,10);

  for(i=0;i<16;i++)
    out[i]=state[i];
}


